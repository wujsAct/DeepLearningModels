...
one-hot: PER [1,0,0,0,0]; LOC [0, 1, 0, 0, 0] ; ORG [0, 0, 1, 0, 0]; MISC [0, 0, 0, 1, 0];
'''
...
'''
sentlent = Shape[0]; seqLent = 250;

allents = 0
right = 0
for i in range(len(NERresult)):
  pred = NERresult[i]
  
  sNoaNo = sentid2aNosNoid[i]
  for item in getNERIndex(pred):
    enti = sNoaNo +"\t"+item
    allents += 1
    if enti in standard_entment:
      right += 1
print allents
print right
print right *1.0/5553
...
for key in standard_entment:
  ...
  for j in xrange(j1,j2):  
    ...
    if getNERTag(NERresult[i][j])!='00001':
      ...
      nerlist.append(getNERTag(NERresult[i][j]))
      temp +=1
      tempj.append([j,getNERTag(NERresult[i][j])])
    else:
      if 'NP' in sents[i][1][j]:
        tempj.append([j,getNERTag(NERresult[i][j])])
        temp+=1
...
#        tempj.append([j,getNERTag(NERresult[i][j])])
#        temp+=1
  print i,NERresult[i],NERresult[i][j1:j2],nerlist
  if temp == j2-j1:
    #print i,j1,j2,standard_entment_name[key],nerlist
    right += 1
