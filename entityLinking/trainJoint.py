...
import time
from model import ctxSum,seqLSTM
...
def getDataTestSets():
  ...
  print msnbcNums
  trainFeatureList = getData("testa")
#  aceTestRet=[]
#  msnbcTestRet=[]
#  TrainRet=[]
...
#    TrainRet.append(aceFeatureList[i])
  return aceFeatureList,msnbcFeatureList,trainFeatureList
...
def main(_):
  ...
  msnbc_num_example = np.shape(msnbc_input)[0]
...
#  lstm_output_msnbc = nerInstance.getEntityRecognition(msnbc_input,msnbc_out)
  ...
  with tf.Session(config=config) as sess:
    modelNER = seqLSTM(args)
    optimizer_NER = tf.train.RMSPropOptimizer(args.learning_rate)
    tvars_NER = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope='seqLSTM_variables')
    grads_NER, _ = tf.clip_by_global_norm(tf.gradients(modelNER.loss, tvars_NER), 10)
    train_op_NER = optimizer_NER.apply_gradients(zip(grads_NER, tvars_NER))
...
#      print "[*] There is no checkpoint for aida"
    ...
    for e in range(200):
      print 'Epoch: %d------------' %(e)
      for ptr in xrange(0,len(train_input),20):
        lstm_output_ace = sess.run(modelNER.output,
                               {modelNER.input_data:ace_input,
                                modelNER.output_data:ace_out,
                                modelNER.num_examples:ace_num_example,
                                modelNER.keep_prob:1.0})
        ...
        lstm_output_msnbc = sess.run(modelNER.output,
                               {modelNER.input_data:msnbc_input,
                                modelNER.output_data:msnbc_out,
                                modelNER.num_examples:msnbc_num_example,
                                modelNER.keep_prob:1.0})
        ent_mention_linking_tag_list,candidate_ent_linking_feature,candidate_ent_type_feature,candidate_ent_prob_feature,\
  ent_mention_lstm_feature,candidate_ent_relcoherent_feature,ent_surfacewordv_feature = \
...
#            modelNEL.save(sess,args.restore,"aida")
        ...
        _,lstm_output = sess.run([train_op_NER,modelNER.output],
                        {modelNER.input_data:train_input[ptr:min(ptr+args.batch_size,len(train_input))],
                         modelNER.output_data:train_out[ptr:min(ptr+args.batch_size,len(train_input))],
                         modelNER.num_examples: num_example,
                         modelNER.keep_prob:1.0})
  ...
  with tf.Session(config=config) as sess:
    ...
    for e in range(100):
      for ptr in xrange(0,len(train_input),args.batch_size):
        lstm_output_testa = sess.run(modelNER.output,
                               {modelNER.input_data:testa_input,
                                modelNER.output_data:testa_out,
                                modelNER.num_examples:testa_num_example,
                                modelNER.keep_prob:1})
        ...
        if accuracy > maximum_linking:
          ...
          #modelNEL.save(sess,args.restore,"aida")
          lstm_output_testb = sess.run(modelNER.output,
                               {modelNER.input_data:testb_input,
                                modelNER.output_data:testb_out,
                                modelNER.num_examples:testb_num_example,
                                modelNER.keep_prob:1})
