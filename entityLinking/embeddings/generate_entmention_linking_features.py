...
def processDescription(line):
  ...
  strs = strs.replace(u'\\n',u' ') #´Ë´¦ÓÐµãÆæ¹ÖÄØ£¡·ñÔò·Ö¾äµÄÊ±ºò»á³öÏÖÎÊÌâ£¡
...
def get_all_candidate_mid_cocurrent(data_flag,ent_Mentions,all_candidate_mids,fout):
  '''
  @2016/12/26 ³éÈ¡ºòÑ¡ÊµÌå¼ä£¬ÊÇ·ñÔÚfreebaseÖÐÓÐ¹²ÏÖ
  '''
  ...
  #for i in tqdm(xrange(145,200)):
    ...
    for j in range(len(ents)):
      ...
      #ÓÐ¸öÐ¡´íÎó²»Ì«ºÃÅÅ³ý£¬ÏÈÕâÑù´¦Àí°É£¡
      if tag == 'NIL':
        if ent_id ==16787:
          ent_id += 2
        else:
      ...
      for mid in cand_mid_dict:    #×¢ÒâÏ¸½ÚµÄ´¦Àí£¬²»È»Êý¾Ý´¦Àí·Ç³£Âé·³ÄØ£¡
        midt = mid.replace(u'/',u'.')[1:]
        new_mid = u'<http://rdf.freebase.com/ns/'+midt+u'>' #/m/0h5k ×ª»»³ÉfreebaseÖÐÍêÕûµÄ¸ñÊ½À²£¡
        #print new_mid
...
def get_candidate_ent_features(data_flag,ent_Mentions,all_candidate_mids,mid2description,descript_Words,mid2figer,f_output,f_output1,f_output2):
  '''
  @2016/12/15  v1: ×îºÃµÄ·½·¨£¬¸ù¾ÝmidÈ¥ÕÒÏà¶ÔÓ¦µÄwikiÒ³Ãæ£¬È»ºó½øÐÐÑµÁ·£¡
  '''
  ent_ment_link_tags = cPickle.load(open('/home/wjs/demo/entityType/informationExtract/data/aida/aida-annotation.p','rb'))
  if data_flag=='train':
    ent_id = 0
  ...
  #for i in tqdm(range(100)):
    ...
    for j in range(len(ents)):
      ...
      for mid in cand_mid_dict:  #½öÓÐdescription,ÊµÌå¹²ÏÖ£¡
        if mid not in mid2description and mid == tag:   #ÓÃÀ´È¥×¥È¡ÐèÒªlinkingµÄÊµÌåÀ²£¡filter to ensure all the result candidates has the description!
        ...
        twordv = np.zeros((100,)) 
        ttypev = np.zeros((113,))
        if mid in mid2figer:         #Ò²ÓÐºÜ¶àÊµÌå²¢Ã»ÓÐfigerµÄtypeËùÒÔÕâ¸öÌØÕ÷Ò²Îª0
          for midtype in mid2figer[mid]:
            ttypev[midtype]=1
           
        if mid in mid2description:  #²»ÐèÒªÕâ¸öÌõ¼þµÄ¹þ£¬ÒòÎªºÜ¶àÊµÌå¶¼È±ÉÙÕâ¸öÌõ¼þ£¬¶ÔÓÚÄÇÐ©²»popularµÄºòÑ¡ÊµÌåÀ²£¡
      ...
      temps_ent_index.append((enti.startIndex,enti.endIndex))  #Í¨¹ýÕâ¸öflagÈ¥³éÈ¡lstm×îºóÒ»²ãµÄÌØÕ÷À²£¡
...
if __name__=='__main__':
  ...
  #get_all_candidate_mid_cocurrent(data_flag,ent_Mentions,all_candidate_mids,f_outputtemp)   #Ö®Ç°ÒÑ¾­´ÓfreebaseÀïÃæ³éÈ¡ÁËµÄ£¡  Ã¿´Îµ÷ÊÔ³ÌÐòµÄÊ±ºò£¬¿ÉÒÔ°ÑÕâ¸ö×¢ÊÍµô£¬ÏÈÓÅÏÈÅÜÒ»±éÕâ¸ö½á¹û£¡
