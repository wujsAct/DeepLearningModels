...
class seqLSTM(Model):
  ...
  def __init__(self,args):
    ...
    print tf.sparse_tensor_to_dense(self.output_data) 
...
#      self.loss = tf.reduce_mean(-self.log_likelihood)
      ...
      prediction = tf.nn.sigmoid(tf.nn.xw_plus_b(output_f,W,b,name="scores"))
      ...
      mask = tf.sign(tf.reduce_max(tf.abs(tf.sparse_tensor_to_dense(self.output_data)), reduction_indices=2))
      cross_entropy *= mask
      cross_entropy = tf.reduce_sum(cross_entropy, reduction_indices=1)
      cross_entropy /= tf.cast(self.length, tf.float32)
      self.loss = tf.reduce_mean(cross_entropy)
      
      correct_prediction = tf.equal(tf.round(tf.nn.sigmoid(self.prediction)), tf.round(tf.sparse_tensor_to_dense(self.output_data)))
